...
You are building the public face of **robertobendi/Anyoat** (modeled after https://www.facebook.com/anyoatdisenyo). Target: GitHub
...
### 0. source/images/ — REAL BRAND ASSETS (mandatory placement)

These are pulled FROM THE ORIGINAL SITE by the ingest phase. They are
the brand's actual photography / logo / illustrations. **Every file
here must land somewhere on the rebuilt site.** Do not relegate them
to a "reference"; place them.

- `source/images/og-image.<ext>` — the page's OG/social-share image.
  On social-media ingest (Facebook, Instagram, LinkedIn, …) this IS
  the profile picture and almost always the logo. BRIEF.md § 3 · Brand
  → Logo asset names how to use it. Default: copy to
  `uploads/logo.<ext>` and place in `<header>` at the size the brief
  picked. Do NOT replace it with a generic SVG wordmark.
- `source/images/favicon.<ext>` — copy to `docs/favicon.<ext>` (the
  export script picks it up if the file is at the workspace root or
  in `uploads/favicon.<ext>`).
- `source/images/img-NN.<ext>` — real product photos / illustrations /
  diagrams pulled from in-page `<img>` tags. Place per BRIEF.md § 5 ·
  Design → Imagery plan: product gallery, about-page photos, hero
  background, blog index. None should be discarded silently. If a
  file is irrelevant to the rebuild scope (e.g. a UI icon that
  duplicates a built-in component), say so in a single HTML comment
  rather than just dropping it.

If a file is genuinely unfit for placement (corrupt, watermarked, of
something the brief explicitly excluded), state that in the inline
review pass at the end — but the default disposition is USE IT.
...
- The header MUST display the real logo (`source/images/og-image.*`)
  if that file exists; if it doesn't, fall back to a typographic
  wordmark in the brief's Display font.
- The hero image, when from `source/images/`, is still subject to
...
user can log into the local admin later):
    ...
    ADMIN_PASSWORD="bis-GvRAoRLdoRxI7g" \
    ADMIN_NAME="Admin" \
    SITE_NAME="Anyoat" \
